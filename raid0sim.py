...
def controleer_bestanden_en_verplaats(src, disk1, disk2, laatste_schijf):
    ...
    print_en_discord(f"\nStart controle van map: {src}", webhook_url)
    print_en_discord(f"Laatste gebruikte schijf was: {laatste_schijf}", webhook_url)
    
    # Verkrijg de lijst van bestanden in de bronmap
    files = [f for f in os.listdir(src) if os.path.isfile(os.path.join(src, f))]
    print_en_discord(f"Aantal gevonden bestanden: {len(files)}", webhook_url)
